...
export default function SubjectEdit({ auth, subject, errors: serverErrors }) {
    const { t } = useTranslation();
    const { data, setData, patch, processing, errors } = useForm({
        name: subject.name,
        description: subject.description || '',
        teacher_name: subject.teacher_name,
        period_of_study: subject.period_of_study,
    ...
    return (
        <Layout auth={auth} title={`${t('edit')} ${subject.name}`}>
            <Box sx={{ p: 3 }}>
                {/* Breadcrumbs */}
                <Breadcrumbs
                                ...
                                <Grid item xs={12}>
                                    <Stack direction="row" spacing={2} sx={{ mt: 3 }}>
                                        <Button
                                            ...
                                            sx={{ minWidth: 200 }}
                                        >
                                            {processing ? t('updatingSubject') : t('updateSubject')}
                                        </Button>
                                        <Button
                                            variant="outlined"
                                            size="large"
